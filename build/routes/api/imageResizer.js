...
imageResizer.get('/', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var name, width, height, fileLocation, fileTarget, targetImage, outputImage;
        ...
        switch (_a.label) {
            case 0:
                ...
                if (handlers.imaExtension(String(name))) {
                    return [2 /*return*/, res.status(400).send("Filename shouldn't include the extension")];
                }
                if (!handlers.exsist(fileTarget)) {
                    return [2 /*return*/, res.status(404).send('Oh uh, image not found')];
                }
                if (!handlers.exsist(fileLocation)) {
                    handlers.makeDir(fileLocation);
                }
                outputImage = fileTarget + "".concat(name, "-thumb-").concat(width, "x").concat(height, ".jpg");
                if (!handlers.exsist(outputImage)) return [3 /*break*/, 1];
                // Caching system
                res.sendFile(outputImage);
                return [3 /*break*/, 3];
            case 1: return [4 /*yield*/, (0, sharp_1.default)(targetImage, outputImage, Number(width), Number(height))];
            case 2:
                _a.sent();
                res.sendFile(outputImage);
                _a.label = 3;
